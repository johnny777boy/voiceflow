    ...
    private var general: some View {
        Form {
            ...
            Text("Puts the offline result in the field the moment it's ready, then upgrades it in place when the AI pass finishes. Off by default — the in-place edit is abandoned if you've started typing, but it hasn't been lived on yet.")
                .font(.caption).foregroundStyle(.secondary)
            Toggle("Release the microphone when idle", isOn: $draft.micIdleReleaseEnabled)
            Text("Turns the mic (and the orange menu-bar indicator) off after 3 minutes without dictating; pressing the hotkey brings it straight back. Only the first dictation after an idle stretch starts the mic at the key-press, so begin speaking a beat after pressing rather than mid-press.")
                .font(.caption).foregroundStyle(.secondary)
